...
class image_converter:
    ...
    def process_img_callback(self,data):
        ...
        while not rospy.is_shutdown():
            if not (self.counter % 1000 == 0):
                continue
            self.counter = self.counter + 1
            try:
                #previously self.bridge.imgmsg_to_cv2(data, "bgr8")
                # self.current_image = self.ros_to_np_img(data)
                # self.previous_img = self.current_image
                self.current_image = data

                # if np.array_equal(self.ros_to_np_img(self.current_image), self.ros_to_np_img(self.previous_img)):
                #     continue
    ...
    def __init__(self):
        ...
        self.image_pub = rospy.Publisher('intruder_detection', Image, queue_size=10)
